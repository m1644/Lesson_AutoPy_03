...
class TestSearchLocators:
    ...
    LOCATOR_CONTACT_BTN = (By.CSS_SELECTOR, "li:nth-child(2)")
    LOCATOR_NAME_FIELD = (By.XPATH, '''//*[@id="contact"]/div[1]/label/input''')
    LOCATOR_EMAIL_FIELD = (By.XPATH, '''//*[@id="contact"]/div[2]/label/input''')
    LOCATOR_CONT_US_FIELD = (By.XPATH, '''//*[@id="contact"]/div[3]/label/span/textarea''')
    LOCATOR_CONT_US_BTN = (By.CSS_SELECTOR, "button")
...
class OperationsHelper(BasePage):
    ...
    def enter_cont_us(self, word):
        logging.info(f'Send "{word}" to element {TestSearchLocators.LOCATOR_CONT_US_FIELD[1]}')
        login_field = self.find_element(TestSearchLocators.LOCATOR_CONT_US_FIELD)
        login_field.clear()
        login_field.send_keys(word)
    
    def click_cont_us_btn(self):
        logging.info("Click CONTACT US button...")
        self.find_element(TestSearchLocators.LOCATOR_CONT_US_BTN).click()

    def get_alert_text(self):
        alert = WebDriverWait(self.driver, 10).until(EC.alert_is_present())
        text = alert.text
        logging.info(f'We find text "{text}" in alert field')  
        alert.accept()
